...
fn ui(f: &mut Frame, app: &mut AtomicVisualizer) {
    ...
    let info_lines = vec![
        Line::from(vec![
            ...
            Span::styled(
                &atom.value,
                Style::default()
                    .fg(if atom.is_prime { Color::Green } else { Color::Red })
                    .add_modifier(Modifier::BOLD)
            ),
            Span::raw(if atom.is_prime { " ✓ PRIME ATOM" } else { " ✗ COMPOSITE ATOM" }),
        ]),
        Line::from(format!("Structure: {} | Electrons: {}", 
    ...
    // Controls
    let controls = vec![
        Line::from("←/→: Switch atom  v: View mode  SPACE: Pause  l: Legend  q: Quit"),
    ];
...
fn render_top_down_atom(f: &mut Frame, area: Rect, atom: &AtomicMembrane, _app: &AtomicVisualizer) {
    ...
    for (layer_idx, layer) in layers.iter().enumerate() {
        ...
        render_electron_shell(f, area, cx, cy, radius as u16, layer, atom.phase, atom.is_prime);
    }
...
fn render_side_view_atom(f: &mut Frame, area: Rect, atom: &AtomicMembrane, _app: &AtomicVisualizer) {
    let cx = area.width / 2;
...
fn render_isometric_atom(f: &mut Frame, area: Rect, atom: &AtomicMembrane, _app: &AtomicVisualizer) {
    let cx = area.width / 2;
    let cy = area.height / 2;
    ...
    for (shell_idx, shell) in shells.iter().enumerate() {
        ...
        for (i, &digit) in shell.iter().enumerate() {
            ...
            if angle.sin() >= 0.0 { // Front half
                let x = cx + (radius as f64 * angle.cos()) as u16 + z_offset;
                let y = cy + (radius as f64 * angle.sin() / 2.0) as u16 - z_offset;
                
                let (ch, color) = electron_appearance(digit, true);
                render_char(f, x, y, ch, 
                    Style::default().fg(color).add_modifier(Modifier::BOLD));
            }
        }
    }
    ...
    render_3d_nucleus(f, area, cx + nucleus_offset, cy - nucleus_offset, atom.is_prime);
}
...
fn render_electron_shell(
    ...
    for angle_deg in (0..360).step_by(10) {
        ...
        if let (Some(xi), Some(yi)) = (to_area_coord(x, area.x, area.width), to_area_coord(y, area.y, area.height)) {
...
fn render_prime_field(f: &mut Frame, area: Rect, cx: u16, cy: u16, radius: u16, phase: f64) {
    ...
    for i in 0..8 {
        ...
        if let (Some(xi), Some(yi)) = (to_area_coord(x, area.x, area.width), to_area_coord(y, area.y, area.height)) {
            ...
            render_char(f, xi, yi, field_char, 
...
fn render_3d_nucleus(f: &mut Frame, area: Rect, cx: u16, cy: u16, is_prime: bool) {
    ...
    for (layer_idx, layer) in layers.iter().enumerate() {
        let opacity = if layer_idx == 0 { Modifier::BOLD } else { Modifier::empty() };
        
        for &(dx, dy, ch) in layer {
            let x = (cx as i16 + dx) as u16;
            let y = (cy as i16 + dy) as u16;
            if x >= area.x && x < area.x + area.width && y >= area.y && y < area.y + area.height {
                render_char(f, x, y, ch, Style::default().fg(color).add_modifier(opacity));
            }
        }
    }
}
...
fn draw_radial_bond(f: &mut Frame, area: Rect, cx: u16, cy: u16, ex: u16, ey: u16, is_prime: bool) {
    ...
    let bond_style = Style::default().fg(if is_prime { Color::Cyan } else { Color::DarkGray });
    ...
    for i in 1..steps {
        ...
        if let (Some(xi), Some(yi)) = (to_area_coord(x, area.x, area.width), to_area_coord(y, area.y, area.height)) {
            let ch = BOND_CHARS[(i as usize * BOND_CHARS.len() / steps as usize).min(BOND_CHARS.len() - 1)];
...
fn render_legend(f: &mut Frame, area: Rect) {
    ...
    for (i, (symbol, desc)) in legend_items.iter().enumerate() {
        ...
        if y < area.y + area.height - 1 {
            ...
            let text = Line::from(vec![Span::styled(line, Style::default().fg(Color::DarkGray))]);
...
fn electron_appearance(digit: char, is_front: bool) -> (char, Color) {
    if digit == '0' {
        ...
        (digit, if is_front { Color::Yellow } else { Color::DarkGray })
    }
}
