...
fn ui(f: &mut Frame, app: &mut AtomExplorer) {
    ...
    let info_text = vec![
        Line::from(vec![
            ...
            Span::styled(
                &atom.membrane,
                Style::default().fg(if atom.is_prime { Color::Green } else { Color::Red })
            ),
            Span::raw(if atom.is_prime { " ✓" } else { " ✗" }),
        ]),
        Line::from(format!("Structure: {}", visualize_structure(&atom.structure))),
    ];
...
fn render_vertical_structure(f: &mut Frame, area: Rect, atom: &PrimeAtom, center_x: u16, center_y: u16) {
...
fn render_orbital_view(f: &mut Frame, area: Rect, atom: &PrimeAtom, center_x: u16, center_y: u16, radius: i32) {
    ...
    while digit_index < atom.structure.len() && orbital_radius < radius {
        ...
        if digit_index < atom.structure.len() {
            ...
            if digit_index + 1 < atom.structure.len() {
                digit_index += 1;
                place_orbital_digit(f, area, center_x, center_y, orbital_radius, atom.structure[digit_index], style, 90);   // Right
            }
            if digit_index + 1 < atom.structure.len() {
                digit_index += 1;
                place_orbital_digit(f, area, center_x, center_y, orbital_radius, atom.structure[digit_index], style, 180);  // Bottom
            }
            if digit_index + 1 < atom.structure.len() {
                digit_index += 1;
                place_orbital_digit(f, area, center_x, center_y, orbital_radius, atom.structure[digit_index], style, 270);  // Left
            }
        }
...
fn place_orbital_digit(f: &mut Frame, area: Rect, center_x: u16, center_y: u16, radius: i32, ch: char, style: Style, angle: i32) {
    let radian = (angle as f32) * std::f32::consts::PI / 180.0;
    let x = center_x as i32 + (radius as f32 * radian.cos()) as i32;
...
fn render_field_lines(f: &mut Frame, area: Rect, center_x: u16, center_y: u16, radius: i32, is_prime: bool) {
    // Draw field lines emanating from the atom
    let field_char = if is_prime { '⚡' } else { '·' };
    let field_color = if is_prime { Color::Green } else { Color::DarkGray };
    ...
    for angle in (0..360).step_by(45) {
        ...
        for r in (radius + 2)..(radius + 5) {
            ...
            if x >= area.x as i32 && x < (area.x + area.width) as i32 &&
               y >= area.y as i32 && y < (area.y + area.height) as i32 {
                let text = Line::from(vec![Span::styled(field_char.to_string(), Style::default().fg(field_color))]);
                let para = Paragraph::new(text);
                let rect = Rect {
                    x: x as u16,
                    y: y as u16,
