    ...
    fn new(membrane: &str) -> Self {
        ...
        while digit_idx < digits.len() {
            ...
            for i in 0..electrons_in_shell {
                let angle =
                    (i as f64 * 360.0 / electrons_in_shell as f64) * std::f64::consts::PI / 180.0;
                let digit = digits[digit_idx + i];
                let is_zero = digit == '0';
                let energy = if is_zero {
                    0.5
                } else {
                    digit.to_digit(10).unwrap_or(0) as f64
                };
...
fn ui(f: &mut Frame, lab: &mut AtomLab) {
    ...
    let controls = vec![Line::from(
        "←/→: Switch atoms  v: View mode  f: Fields  b: Bonds  o: Orbitals  q: Quit",
    )];
    ...
    for shell in &atom.electron_shells {
        ...
        for electron in &shell.electrons {
            ...
            if let (Some(xi), Some(yi)) = (
                to_buffer_coord(x, buffer[0].len()),
                to_buffer_coord(y, buffer.len()),
            ) {
...
fn render_vertical_atom(buffer: &mut Vec<Vec<char>>, atom: &MembraneAtom, cx: f64, cy: f64) {
    ...
    for (i, &digit) in atom.digits.iter().enumerate() {
        ...
        if let (Some(xi), Some(yi)) = (
            to_buffer_coord(cx, buffer[0].len()),
            to_buffer_coord(y, buffer.len()),
        ) {
            buffer[yi][xi] = digit;
