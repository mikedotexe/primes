...
fn main() {
    ...
    results.sort_by(|a, b| {
        b.lagrange_success_rate
            .partial_cmp(&a.lagrange_success_rate)
            .unwrap()
    });

    // Display results
    println!(
        "{:<15} {:<15} {:>10} {:>10} {:>12} {:>12} {:>12}",
    ...
    let high_performers: Vec<_> = results
        .iter()
        .filter(|r| r.lagrange_success_rate > 10.0)
        .collect();
    let low_performers: Vec<_> = results
        .iter()
        .filter(|r| r.lagrange_success_rate < 5.0)
        .collect();
    ...
    // Calculate averages
    let high_avg_product: f64 = high_performers
        .iter()
        .map(|r| r.digit_product as f64)
        .sum::<f64>()
        / high_performers.len() as f64;
    let low_avg_product: f64 = low_performers
        .iter()
        .map(|r| r.digit_product as f64)
        .sum::<f64>()
        / low_performers.len() as f64;

    let high_avg_zeros: f64 = high_performers
        .iter()
        .map(|r| (r.zero_density1 + r.zero_density2) / 2.0 * 100.0)
        .sum::<f64>()
        / high_performers.len() as f64;
    let low_avg_zeros: f64 = low_performers
        .iter()
        .map(|r| (r.zero_density1 + r.zero_density2) / 2.0 * 100.0)
        .sum::<f64>()
        / low_performers.len() as f64;
    ...
    println!("\n📈 STATISTICAL SUMMARY:");
    println!(
        "   High performers: avg product = {:.1}, avg zero% = {:.1}%",
        high_avg_product, high_avg_zeros
    );
    println!(
        "   Low performers:  avg product = {:.1}, avg zero% = {:.1}%",
        low_avg_product, low_avg_zeros
    );
