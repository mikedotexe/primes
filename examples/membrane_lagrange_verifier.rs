...
fn main() {
    ...
    println!(
        "Structural L1: {} = {}",
        structural_l1,
        visualize_membrane(&structural_l1.to_string())
    );
    println!(
        "Is prime: {}",
        if is_prime(&structural_l1) {
            "✓"
        } else {
            "✗"
        }
    ...
    println!(
        "\nMidpoint: {} = {}",
        mid34,
        visualize_membrane(&mid34.to_string())
    );
    ...
    println!(
        "Structural: {} = {}",
        structural_mid,
        visualize_membrane(&structural_mid.to_string())
    );
    println!(
        "Is prime: {}",
        if is_prime(&structural_mid) {
            "✓"
        } else {
            "✗"
        }
...
fn analyze_lagrange_structure(value: &str, config: &MembraneConfig) -> bool {
    ...
    let expected_len = 2 * (1 + config.k_outer as usize + 1 + config.k_inner as usize)
        + 1
    ...
    println!(
        "               Zeros: {}/{} padding positions",
        zero_count, total_padding
    );
