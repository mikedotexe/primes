...
fn deep_dive_configuration(body1: &str, body2: &str, max_space: usize) {
    ...
    println!("{}", "=".repeat(60));
    println!("Body 1: {} ({})", body1, if is_prime(&b1) { "PRIME" } else { "COMPOSITE" });
    println!("Body 2: {} ({})", body2, if is_prime(&b2) { "PRIME" } else { "COMPOSITE" });
    ...
    for space_size in 1..=max_space {
        ...
        println!("\nSpace size {}: {}/{} = {:.1}% success", 
            space_size, primes_found.len(), total_tests, success_rate);
...
fn test_composite_patterns() {
    ...
    println!("\n📐 PERFECT SQUARES:");
    let squares = [("4", "2²"), ("9", "3²"), ("16", "4²"), ("25", "5²"), ("36", "6²")];
    for (num, desc) in &squares {
        let n = num.parse::<BigUint>().unwrap();
        println!("{} = {} ({})", num, desc, if is_prime(&n) { "prime" } else { "composite" });
    }
    
    // Test products of primes
    println!("\n✖️ PRODUCTS OF PRIMES:");
    let products = [("6", "2×3"), ("10", "2×5"), ("14", "2×7"), ("15", "3×5"), ("21", "3×7")];
...
fn systematic_composite_test() {
    ...
    let composites = ["4", "6", "8", "9", "10", "12", "14", "15", "16", "18", "20", "21"];
    ...
    println!("\n2️⃣ PRIME ↔ PRIME²:");
    let prime_squares = [("2", "4"), ("3", "9"), ("5", "25"), ("7", "49"), ("11", "121")];
    ...
    for (prime, square) in &prime_squares {
        ...
        println!("   {} ↔ {}: {}/9 = {:.1}%", prime, square, count, (count as f64 / 9.0) * 100.0);
    }
}
