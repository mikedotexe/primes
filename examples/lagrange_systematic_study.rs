...
fn analyze_patterns(data: &[LagrangeDataPoint]) {
    ...
    for point in data {
        ...
        digit_entry.0 += 1;
        if point.result_prime { digit_entry.1 += 1; }
    ...
    for (zeros, (total, primes)) in &by_zero_count {
        let rate = (*primes as f64 / *total as f64) * 100.0;
        println!("   {} zeros in bodies: {}/{} = {:.1}%", zeros, primes, total, rate);
    }
...
fn output_prime_wall(data: &[LagrangeDataPoint]) {
    ...
    println!("Found {} primes from {} tests ({:.1}% success)",
        primes.len(), data.len(), 
...
fn main() {
    ...
    for point in &all_data {
        ...
        entry.0 += 1;
        if point.result_prime { entry.1 += 1; }
    }
    
    let mut sorted_configs: Vec<_> = config_success.iter()
    ...
    for space in 1..=3 {
        for pos in 0..space {
            ...
            println!("   Space={}, Pos={}: Best digit={} ({:.1}% success)",
                space, pos, best_digit, rate * 100.0);
        }
    }
}