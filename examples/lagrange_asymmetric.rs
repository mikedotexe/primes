...
fn test_small_large_pair() {
    ...
    println!(
        "Small body: {} ({})",
        small_prime,
        prime_status(small_prime)
    );
    println!(
        "Large body: {} ({})",
        large_prime,
        prime_status(large_prime)
    );
...
fn test_tiny_giant_pair() {
    ...
    if lagrange_points.is_empty() {
        ...
        for (pos, digit) in &lagrange_points {
            ...
            println!(
                "  Visual: {}",
                create_detailed_visual(tiny, &buffer_str, giant, *pos)
            );
            ...
            for digit in 1..=9 {
                ...
                if let Ok(num) = BigUint::from_str(&full) {
                    if is_prime(&num) {
                        ...
                        if found_count == 1 {
                            println!(
                                "  Example L-point: pos {}, digit {} → {}-digit PRIME",
                                position,
                                digit,
                                full.len()
                            );
