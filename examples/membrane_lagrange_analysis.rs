...
fn main() {
    ...
    let tests = vec![
        ...
        (303050303u64, 303110303u64, "Same structure, middle: 5 vs 11"),
    ];
    ...
    for (p1_val, p2_val, desc) in tests {
        ...
        // Numeric midpoint
        let mid = (&p1 + &p2) / 2u8;
        println!("Numeric midpoint: {} = {}", mid, visualize(&mid.to_string()));
        println!("                  Prime: {}", if is_prime(&mid) { "✓" } else { "✗" });
    ...
    for val in zero_candidates {
        ...
        println!("           Prime: {}", if is_prime(&num) { "✓" } else { "✗" });
    ...
    for (method, value) in methods {
        println!("\n{}: {} = {}", method, value, visualize(&value.to_string()));
        println!("{} Prime: {}", " ".repeat(method.len()), if is_prime(&value) { "✓" } else { "✗" });
    }
    ...
    println!("\nMidpoint: {} = {}", large_mid, visualize(&large_mid.to_string()));
    
    // What if we zero out the middle section?
    let zero_middle = BigUint::from(30070000700703u64);
    println!("All-zero: {} = {}", zero_middle, visualize(&zero_middle.to_string()));
    println!("          Prime: {}", if is_prime(&zero_middle) { "✓" } else { "✗" });
...
fn visualize(s: &str) -> String {
    s.chars().map(|c| if c == '0' { '◯' } else { c }).collect::<Vec<_>>().into_iter().map(|c| c.to_string()).collect::<Vec<_>>().join("-")
}
...
fn analyze_midpoint_structure(p1: &BigUint, p2: &BigUint, mid: &BigUint) {
    ...
    if s1.len() == s2.len() && s1.len() == sm.len() {
        ...
        if diffs.len() == 1 {
            ...
            println!("                  Structure preserved except position {}: {} ↔ {} → {}", 
                pos, c1, c2, cm);
            
            // Check if it's the average
            if let (Some(d1), Some(d2), Some(dm)) = (c1.to_digit(10), c2.to_digit(10), cm.to_digit(10)) {
                let avg = (d1 + d2) / 2;
                if dm == avg {
