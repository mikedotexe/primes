...
fn visual_structure_demo() {
    ...
    let patterns = vec![
        ("Classic", MembraneConfig::new(10, 3, 7, 0, 0), 5, "307050703"),
        ("Breathing", MembraneConfig::new(10, 3, 3, 0, 1), 5, "3305033"),
        ("Symmetric", MembraneConfig::new(10, 3, 3, 1, 1), 5, "303050303"),
        ("Exclusive", MembraneConfig::new(10, 3, 7, 1, 1), 5, "307050703"),
    ];
...
fn statistical_patterns() {
    ...
    for (name, base, outer, inner, k_outer, k_inner) in test_configs {
        ...
        println!("{} - ({},{}) k=({},{}):", name, outer, inner, k_outer, k_inner);
        println!("  Success rate: {}/{} = {:.1}%", 
            found, test_count, found as f64 / test_count as f64 * 100.0);
        
        for (seed, membrane, decimal) in examples {
            println!("  Seed {:2}: {} (base {}) = {}", seed, membrane, base, decimal);
        }
        println!();
    }
}
...
fn lagrange_showcase() {
    ...
    println!("Prime 1 (Earth): {} = {}", prime1, visualize_number(&prime1.to_string()));
    println!("Prime 2 (Moon):  {} = {}", prime2, visualize_number(&prime2.to_string()));
    println!();
    ...
    for (pos, digit, name) in lagrange_configs {
        ...
        println!("  Buffer: {}", buffer_str.chars().enumerate().map(|(i, c)| {
            if i == pos { format!("[{}]", c) } else { c.to_string() }
        }).collect::<String>());
        println!("  Full: {} = {} digits", concatenated, concatenated_str.len());
        println!("  Prime: {}", if is_prime(&concatenated) { "✓ YES!" } else { "✗" });
    }
}
...
fn visualize_number(s: &str) -> String {
    s.chars().map(|c| if c == '0' { '◯' } else { c }).map(|c| c.to_string()).collect::<Vec<_>>().join("-")
}