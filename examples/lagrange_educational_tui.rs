...
use prime_physics_engine::{
    ...
    gravity::{PrimeParticle, PhysicsCache},
    is_prime,
};
...
impl Default for EducationalState {
    fn default() -> Self {
        let explanations = vec![
            ...
            "L1 between membrane primes preserves structure, averaging only the middle digit.".to_string(),
...
impl EducationalState {
    fn generate_large_primes(&mut self) {
        ...
        self.status_message = "Large primes generated! Press 'a' to animate, 'e' to cycle explanations".to_string();
    }
    ...
    fn calculate_detailed_lagrange_points(&mut self) {
        ...
        if let (Some(p1), Some(p2)) = (&self.prime1, &self.prime2) {
            ...
            self.lagrange_points.push(LagrangePointInfo {
                ...
                explanation: "L1: Between primes - unstable equilibrium. Small perturbations grow.".to_string(),
                stability: 0.3,
            ...
            self.lagrange_points.push(LagrangePointInfo {
                ...
                explanation: "L2: Beyond smaller prime - shielded from larger prime's influence.".to_string(),
                stability: 0.2,
            ...
            self.lagrange_points.push(LagrangePointInfo {
                ...
                explanation: "L4: Leading trojan - stable! Forms equilateral triangle with primes.".to_string(),
                stability: 0.8,
            ...
            self.lagrange_points.push(LagrangePointInfo {
                ...
                explanation: "L5: Trailing trojan - equally stable. Natural parking orbit.".to_string(),
                stability: 0.8,
                dominance: "Shared".to_string(),
            });
        }
...
fn draw_educational_ui(f: &mut Frame, state: &EducationalState) {
    ...
    let header = Paragraph::new(Line::from(vec![
        ...
        Span::styled("Lagrange Points in Prime Space", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        Span::styled(" - Understanding Gravitational Equilibrium", Style::default().fg(Color::Gray)),
    ]))
...
fn draw_prime_visualization(f: &mut Frame, state: &EducationalState, area: Rect) {
    ...
    if let Some(p1) = &state.prime1 {
        ...
        let config_str = format!("({},{}) k=({},{}) base {}", 
            state.config1.outer, state.config1.inner,
            state.config1.k_outer, state.config1.k_inner,
            state.config1.base
        );
    ...
    if let Some(p2) = &state.prime2 {
        ...
        let config_str = format!("({},{}) k=({},{}) base {}", 
            state.config2.outer, state.config2.inner,
            state.config2.k_outer, state.config2.k_inner,
            state.config2.base
        );
...
fn draw_lagrange_diagram(f: &mut Frame, state: &EducationalState, area: Rect) {
    ...
    if state.prime1.is_some() && state.prime2.is_some() {
        ...
        for lpoint in &state.lagrange_points {
            ...
            if lpoint.is_prime {
                spans.push(Span::styled(" ✓ PRIME!", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)));
            }
    ...
    let diagram_widget = Paragraph::new(lines)
        .block(Block::default().borders(Borders::ALL).title("🌌 Gravitational Map"));
...
fn draw_explanations(f: &mut Frame, state: &EducationalState, area: Rect) {
    ...
    let explanation = Paragraph::new(vec![
        Line::from(vec![
            Span::styled("💡 ", Style::default().fg(Color::Yellow)),
            Span::styled("Understanding Lagrange Points", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(""),
        ...
        Line::from(vec![
            Span::raw("Current: "),
            Span::styled(
                format!("{}/{}", state.current_explanation + 1, state.explanations.len()),
    ...
    if let Some(lpoint) = state.lagrange_points.get(0) {
        ...
        .block(Block::default().borders(Borders::ALL).title("Point Details"));
...
fn interpolate_value(v1: &BigUint, v2: &BigUint, t: f64) -> BigUint {
    ...
    if (t - 0.5).abs() < 0.01 && s1.len() == s2.len() {
        ...
        let diffs: Vec<_> = s1.chars().zip(s2.chars()).enumerate()
        ...
        if diffs.len() == 1 {
            ...
            let pos = diffs[0];
            if let (Some(d1), Some(d2)) = (s1.chars().nth(pos).unwrap().to_digit(10), 
