...
fn main() {
    ...
    for buffer_size in [0, 1, 3, 5, 7, 9, 11] {
        ...
        println!("\nBuffer size {}: {}", buffer_size, visualize_concatenation(&prime1.to_string(), &zeros, &prime2.to_string()));
        println!("Full number: {}", concatenated_str);
        println!("Is prime: {}", if is_prime(&concatenated) { "✓ YES!" } else { "✗ No" });
    ...
    for position in 0..buffer_size {
        for digit in 1..=9 {
            ...
            if is_prime(&concatenated) {
                println!("🌟 FOUND! Position {}, digit {}: {}", 
                    position, digit, 
                    visualize_with_highlight(&prime1.to_string(), &buffer_str, &prime2.to_string(), position));
    ...
    for pattern in patterns {
        ...
        println!("\nPattern: {}", visualize_concatenation(&prime1.to_string(), pattern, &prime2.to_string()));
        println!("Is prime: {}", if is_prime(&concatenated) { "✓ YES!" } else { "✗ No" });
    }
...
fn visualize_concatenation(p1: &str, buffer: &str, p2: &str) -> String {
    format!("{} [{}] {}", 
        p1.chars().map(|c| if c == '0' { '◯' } else { c }).collect::<String>(),
        buffer.chars().map(|c| if c == '0' { '◯' } else { c }).collect::<String>(),
        p2.chars().map(|c| if c == '0' { '◯' } else { c }).collect::<String>()
    )
}
    ...
    for position in 0..buffer_size {
        for digit in 1..=9 {
            ...
            if is_prime(&concatenated) {
                ...
                println!("    L{}: position {}, mass {}", position + 1, position, digit);
            }
        }
    }
