...
fn visualize_lagrange_field(analysis: &LagrangeAnalysis) {
    ...
    println!(
        "   Prime-creating points: {}",
        analysis
            .lagrange_points
            .iter()
            .filter(|lp| lp.creates_prime)
            .count()
    );
    ...
    for pos in 0..analysis.space_size {
        if let Some(digits) = position_map.get(&pos) {
            println!(
                "   Position {}: {} Lagrange points (digits: {:?})",
                pos,
                digits.len(),
                digits
            );
