...
fn ui(f: &mut Frame, app: &LagrangeDemo) {
    ...
    let viz_widget = Paragraph::new(viz)
        .block(Block::default().borders(Borders::ALL).title("Concatenated Prime System"));
    ...
    let controls = vec![
        Line::from(vec![
            Span::raw("Position: "),
            Span::styled(format!("{}", app.current_position), Style::default().fg(Color::Yellow)),
            Span::raw("  Digit: "),
            Span::styled(format!("{}", app.current_digit), Style::default().fg(Color::Yellow)),
        ]),
        Line::from(""),
    ...
    if app.show_help {
        ...
        let help_widget = Paragraph::new(help_text)
            .block(Block::default().borders(Borders::ALL).title("Help (h to toggle)"))
...
fn create_visualization(app: &LagrangeDemo) -> Text {
    ...
    lines.push(Line::from(vec![
        ...
        Span::raw(format!(" ({} digits, {})", app.prime1.len(), if p1_is_prime { "✓ prime" } else { "✗ not prime" })),
    ]));
    lines.push(Line::from(vec![
        Span::raw("Body 2: "),
        Span::styled(&app.prime2, Style::default().fg(Color::Yellow)),
        Span::raw(format!(" ({} digits, {})", app.prime2.len(), if p2_is_prime { "✓ prime" } else { "✗ not prime" })),
    ]));
    lines.push(Line::from(vec![
    ...
    spans.push(Span::raw(" | "));
    spans.push(Span::styled(&app.prime2, Style::default().fg(Color::Yellow)));
    ...
    lines.push(Line::from(vec![
        Span::raw("Length: "),
        Span::styled(format!("{} digits", full_string.len()), Style::default().fg(Color::Magenta)),
    ]));
    lines.push(Line::from(""));
