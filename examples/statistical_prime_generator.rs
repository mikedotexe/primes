...
fn generate_with_best_config(configs: &[ConfigStats]) {
    ...
    let best = &configs[0];
    println!("Using: Base {} ({},{}) k=({},{}) - {:.1}% success rate",
        best.base, best.outer, best.inner, best.k_outer, best.k_inner,
...
fn statistical_batch_generation(configs: &[ConfigStats]) {
    ...
    for _ in 0..100 {
        ...
        if !selected.successful_seeds.is_empty() {
            let seed = selected.successful_seeds
                .choose(&mut rng)
                .copied()
                .unwrap_or(0);
    ...
    println!("Results:");
    println!("  Total primes: {} / 100 ({:.0}%)", primes_found, primes_found as f64);
    println!("\nBreakdown by base:");
...
fn generate_concatenated_primes() {
    ...
    for (buffer_size, position, digit) in successful_lagrange_configs {
        ...
        println!("\nBuffer: {} (L{} = {})", 
            buffer_str.chars().enumerate().map(|(i, c)| {
                if i == position { format!("[{}]", c) } else { c.to_string() }
            }).collect::<String>(),
            position + 1,
            digit
        );
        println!("Full: {}", concatenated_str);
        println!("Prime: {}", if is_prime(&concatenated) { "✓ YES!" } else { "✗ No" });
    }
