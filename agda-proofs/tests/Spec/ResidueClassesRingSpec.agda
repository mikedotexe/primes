...
{-|
  ...
  For each ring axiom (associativity, commutativity, identity), we:
    ...
    3. Verify the result normalizes to refl

  Success means: the axiom is not just proven abstractly, but computes
  correctly on actual values.
...
open import ResidueClassesComplete
...
{-|
  ...
  concrete values. If the theorem computes correctly:
    1. Left side:  (A10 ⊕ B10) ⊕ C10 = ([3] ⊕ [7]) ⊕ [9] = [0] ⊕ [9] = [9]
    2. Right side: A10 ⊕ (B10 ⊕ C10) = [3] ⊕ ([7] ⊕ [9]) = [3] ⊕ [6] = [9]
    3. Both sides compute to [9], so they're equal by refl

  If this doesn't normalize, the theorem has an error.
-}
...
{-|
  ...
  These tests should compile once ResidueClassesComplete is proven.
  Some postulates remain in that module (mod-distribˡ-+, etc.) but the
  ring structure proofs themselves are complete.
