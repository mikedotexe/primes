...
modℕ : (n d : ℕ) .{{_ : NonZero d}} → ℕ
modℕ n d = toℕ (n mod d)
...
freqs : (base : ℕ) → (d : ℕ) .{{_ : NonZero d}} → List ℕ
freqs base d = map count-residue (upto d)
  where
    -- Count how many digits have residue r
    count-residue : ℕ → ℕ
    count-residue r = count r (map (λ n → modℕ n d) (upto base))
