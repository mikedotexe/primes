...
impl Pattern {
    fn total_len(&self) -> usize {
        ...
        let side: usize = self.layers.iter().map(|ly| ly.zero + ly.slot).sum();
        mid + 2 * side
    }
}
...
fn parse_midpoint(s: &str) -> Midpoint {
    ...
    assert!(
        parts.len() == 2,
        "midpoint must be free:<len> or zeros:<len>"
    );
...
fn parse_u32_list(s: &str) -> Vec<u32> {
    if s.trim().is_empty() {
        ...
        s.split(',')
            .map(|d| u32::from_str(d.trim()).unwrap())
            .collect()
    }
}
...
fn validate_allowed_last_digits(base: u32, allowed: &[u32]) {
    for &d in allowed {
        assert!(
            d < base,
            "allowed_last_digits contains digit {} >= base {}",
            d,
            base
        );
    }
...
fn build_digit_spec(p: &Pattern) -> Vec<Option<Vec<u32>>> {
    ...
    fn set_open_slots(
        ...
        let last_set: Option<HashSet<u32>> = if allowed_last_digits.is_empty() {
            None
        } else {
            Some(allowed_last_digits.iter().cloned().collect())
        };
        ...
        for idx in range {
            ...
            if idx == 0 {
                ...
                if mirror {
                    if let Some(ref set) = last_set {
                        allowed.retain(|d| set.contains(d));
                    }
                }
            }
    ...
    match p.midpoint {
        Midpoint::Zeros(_) => {
            for slot in &mut spec[mid_start..mid_end] {
                *slot = None;
            }
        }
        Midpoint::Free(_) => {
    ...
    for layer in p.layers.iter() {
        if layer.zero > 0 {
            ...
            for slot in &mut spec[lz_start..left_end] {
                *slot = None;
            }
            left_end = lz_start;

            let rz_end = right_start + layer.zero;
            for slot in &mut spec[right_start..rz_end] {
                *slot = None;
            }
            right_start = rz_end;
        }
...
fn residue_null_probability_with_spec(spec: &DigitSpec, base: u32, modm: u32) -> f64 {
    ...
    for (i, allowed_opt) in spec.slots.iter().enumerate() {
        if let Some(allowed) = allowed_opt {
            ...
            next.fill(0.0);
            for (r, &v) in dist.iter().enumerate().take(m) {
                if v == 0.0 {
                    continue;
                }
                for &d in allowed {
...
fn union_null_probability_lcm_with_spec(spec: &DigitSpec, base: u32, track: &[u32]) -> (u32, f64) {
    ...
    if l > LCM_CAP {
        let mut prod = 1.0;
        for &m in &mods {
            prod *= 1.0 - residue_null_probability_with_spec(spec, base, m);
        }
    ...
    for &pmod in &mods {
        for r in (0..m).step_by(pmod as usize) {
            is_bad[r] = true;
        }
    }
    let mut p_any = 0.0;
...
fn expected_density_local_with_spec(p: &Pattern, spec: &DigitSpec, track: &[u32]) -> f64 {
    ...
    if p.mirror && p.total_len().is_multiple_of(2) {
        return 0.0;
    }
    let mut prod = 1.0;
    for &m in track {
        if m >= 2 {
            prod *= 1.0 - residue_null_probability_with_spec(spec, p.base, m);
        }
    }
    prod * (1.0 / ln_x)
}
...
fn lcm_list(mods: &[u32]) -> u32 {
    mods.iter().copied().filter(|&m| m >= 2).fold(1u32, lcm_u32)
}
...
fn residue_null_probability(p: &Pattern, modm: u32) -> f64 {
    ...
    for (i, w) in weights.iter_mut().enumerate() {
        let exp = n - 1 - i;
        *w = pow_mod_u32(base, exp, modm);
    }

    let mut dist = vec![0.0f64; m];
    dist[0] = 1.0;
    for (idx, allowed_opt) in spec.iter().enumerate() {
        if let Some(allowed) = allowed_opt {
            if allowed.is_empty() {
                continue;
            }
...
fn union_null_probability_lcm(p: &Pattern, track: &[u32]) -> (u32, f64) {
    ...
    if l > LCM_CAP {
        let mut prod = 1.0;
        for &m in &mods {
            prod *= 1.0 - residue_null_probability(p, m);
        }
        return (0, (1.0 - prod).clamp(0.0, 1.0));
    }
...
fn auto_select_moduli(
    ...
    for m in sieve_primes_u32(pmax) {
        cand.push((m, residue_null_probability(pat, m)));
    }
    cand.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
    let mut out: Vec<u32> = cand
        .iter()
        .filter(|(_, p0)| *p0 >= min_p0)
        .take(k)
        .map(|(m, _)| *m)
        .collect();
    if out.len() < k {
        for (m, _) in cand.iter() {
            if out.len() >= k {
                break;
            }
            if !out.contains(m) {
...
fn auto_select_moduli_global_grid(
    ...
    for mlen in mid.0..=mid.1 {
        for iz0 in iz.0..=iz.1 {
            ...
            let p = Pattern {
                ...
                layers,
                constraints: Constraints {
                    allowed_last_digits: allowed.to_vec(),
    ...
    let mut out: Vec<u32> = vec_sc
        .into_iter()
        .filter(|(_, s)| *s >= min_p0 * cells)
        .take(k)
        .map(|(m, _)| m)
        .collect();
    if mirror {
...
fn auto_select_moduli_global_line(
    ...
    for &v in vals {
        ...
        let p = Pattern {
            ...
            } else if mid_kind == "free" {
                Midpoint::Free(fixed_mid)
            } else {
                Midpoint::Zeros(fixed_mid)
            },
            layers,
            constraints: Constraints {
                allowed_last_digits: allowed.to_vec(),
...
fn explain_for_pattern(p: &Pattern, track: &[u32]) -> ExplainReport {
    let spec = build_digit_spec(p);
    let n = spec.len();
    let open_mask: Vec<u8> = spec
        .iter()
        .map(|s| if s.is_some() { 1 } else { 0 })
        .collect();
    let allowed_lens: Vec<usize> = spec
        .iter()
    ...
    ExplainReport {
        ...
        mid_len: match p.midpoint {
            Midpoint::Free(l) | Midpoint::Zeros(l) => l,
        },
        inner_zero: p.layers.first().map(|ly| ly.zero).unwrap_or(0),
        open_mask,
        allowed_lens,
...
fn do_sample(
    ...
    let inner_zero = p.layers.first().map(|ly| ly.zero).unwrap_or(0);
    let spec = build_digit_spec(p);
    let spec_wrapped = build_spec(p);
        ...
        (0..samples)
            ...
            .map(|i| {
                ...
                for (j, &m) in track.iter().enumerate() {
                    if m >= 2 && horner_mod(&digits, p.base, m) == 0 {
                        div[j] = 1;
                        if pre_sieve {
                            blocked = true;
                        }
    ...
    let enrichment_vs_local_exact = if expected_local_exact > 0.0 {
        density / expected_local_exact
    } else if primes == 0 {
        0.0
    } else {
        f64::INFINITY
    ...
    match cli.command {
        ...
        Commands::Sample {
            ...
            let p = Pattern {
                ...
                constraints: Constraints {
                    allowed_last_digits: parse_allowed_last_digits(cli.base, &allowed_last_digits),
                    forbid_leading_zero: true,
                },
                mirror,
            };
                ...
                combos
                    ...
                    .map(|(m, z)| {
                        ...
                        let p = Pattern {
                            ...
                            layers,
                            constraints: Constraints {
                                allowed_last_digits: parse_allowed_last_digits(
                                    cli.base,
                                    &allowed_last_digits,
                                ),
                                forbid_leading_zero: true,
                        ...
                        if model_only {
                                ...
                                residue_null_vector_via_lcm_with_spec(&spec_wrapped, p.base, &track)
                            {
                                ...
                                track
                                    ...
                                    .map(|&m| {
                                        if m >= 2 {
                                            residue_null_probability_with_spec(
                                                &spec_wrapped,
                                                p.base,
                                                m,
                                            )
            ...
            for mid_len in mid_a..=mid_b {
                ...
                for iz in iz_a..=iz_b {
                    ...
                    let p = Pattern {
                        ...
                        layers,
                        constraints: Constraints {
                            allowed_last_digits: allowed.clone(),
                            forbid_leading_zero: true,
                        },
                        mirror,
                    };
                ...
                match job {
                    Job::Sample {
                        ...
                        if allowed_last_digits.trim().eq_ignore_ascii_case("auto") {
                            eprintln!(
                                "Auto-selected coprime digits for base {}: {:?}",
                                job_base, allowed
                            );
                        }
                        ...
                        if enable_at {
                            eprintln!(
                                "Auto-track: selected {} moduli up to p={}",
                                track.len(),
                                pmax
                            );
                    ...
                    Job::Grid {
                        ...
                        let global_track = if enable_at && at_mode == AutoMode::Global {
                            ...
                            eprintln!(
                                "Auto-track (global mode): selected {} moduli up to p={}",
                                track.len(),
                                pmax
                            );
                            eprintln!("Tracked moduli: {:?}", track);
                    ...
                    Job::ModelOnly {
                        ...
                        for mid_len in mid_a..=mid_b {
                            for iz in iz_a..=iz_b {
                                ...
                                let spec_wrapped = build_spec(&p);
                                let model_p0: Vec<f64> = if let Some((_l, pairs)) =
                                    residue_null_vector_via_lcm_with_spec(
                                        &spec_wrapped,
                                        p.base,
                                        &track,
                                    ) {
                            ...
                            vals.par_iter()
                                .map(|&v| {
                                    ...
                                    if model_only {
                                        ...
                                        let expected_pnt = expected_density_pnt_conditional(&p);
                                        let expected_local = expected_density_local(&p, &track);
                                        let expected_local_exact =
                                            expected_density_local_exact(&p, &track);
                                        SampleReport {
                                            pattern: format_pattern(&p),
                                            base: p.base,
                                            total_len: p.total_len(),
...
mod tests {
    ...
    fn residue_model_matches_mc_mod3() {
        let p = Pattern {
            ...
            constraints: Constraints {
                allowed_last_digits: vec![1, 3, 7, 9],
                forbid_leading_zero: true,
            },
            mirror: false,
        };
        ...
        for _ in 0..trials {
            ...
            if (&n % BigUint::from(3u32)).is_zero() {
                hits += 1;
            }
        }
    ...
    fn mirror_even_length_divisible_by_b_plus_1() {
        let p = Pattern {
            ...
            constraints: Constraints {
                allowed_last_digits: vec![1, 5, 7, 11],
                forbid_leading_zero: true,
            },
            mirror: true,
        };
        ...
        for _ in 0..1000 {
            let n = super::sample_number(&p, &spec, &mut rng);
            assert!(
                (&n % BigUint::from(13u32)).is_zero(),
                "palindrome not divisible by b+1"
            );
        }
    }
    ...
    fn mirror_odd_length_not_always_divisible_by_b_plus_1() {
        ...
        for _ in 0..200 {
            let n = super::sample_number(&p, &spec, &mut rng);
            if !(&n % BigUint::from(13u32)).is_zero() {
                all_divisible = false;
                break;
            }
        }
        assert!(
            !all_divisible,
            "odd-length palindromes should not all be divisible by b+1"
        );
    }
    ...
    fn p0_from_lcm_equals_per_prime_when_under_cap() {
        ...
        let (l, vecp) =
            residue_null_vector_via_lcm_with_spec(&spec, p.base, &track).expect("under cap");
        assert_eq!(l, 15);
        for (pm, p0) in vecp {
