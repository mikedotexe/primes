...
impl WasmMembraneEngine {
    ...
    pub fn new(base: u32, outer: u32, inner: u32, k_outer: u32, k_inner: u32) -> Self {
        Self { base, outer, inner, k_outer, k_inner }
    }
    
    /// Generate a prime candidate from a seed
    #[wasm_bindgen]
    pub fn generate(&self, seed: u8) -> WasmGenerationResult {
        let config = MembraneConfig::new(self.base, self.outer, self.inner, self.k_outer, self.k_inner);
...
pub fn get_best_configs(base: u32) -> String {
    let configs = match base {
        6 => vec![
            BestConfig { outer: 1, inner: 5, k_outer: 0, k_inner: 0, success_rate: 33.0 },
            BestConfig { outer: 5, inner: 1, k_outer: 0, k_inner: 0, success_rate: 31.0 },
        ],
        10 => vec![
            BestConfig { outer: 3, inner: 7, k_outer: 0, k_inner: 0, success_rate: 20.0 },
            BestConfig { outer: 7, inner: 3, k_outer: 0, k_inner: 0, success_rate: 19.5 },
        ],
        12 => vec![
            BestConfig { outer: 5, inner: 7, k_outer: 0, k_inner: 0, success_rate: 25.0 },
            BestConfig { outer: 7, inner: 5, k_outer: 0, k_inner: 0, success_rate: 24.5 },
        ],
        _ => vec![
            BestConfig { outer: 1, inner: base-1, k_outer: 0, k_inner: 0, success_rate: 15.0 },
        ],
    };
...
#[wasm_bindgen]
pub fn generate_primes_batch(base: u32, outer: u32, inner: u32, count: u32) -> Result<Vec<JsValue>, JsValue> {
