...
use crate::gravity::{PrimeParticle, GravitationalField};
...
impl ClusterAnalysis {
    ...
    pub fn new(particles: &[PrimeParticle], _field: &GravitationalField) -> PhysicsResult<Self> {
        ...
        if particles.len() > MAX_PARTICLES {
            return Err(PhysicsError::LagrangeError(
                format!("Too many particles to analyze: {} (max: {})", 
        ...
        for (i, particle) in particles.iter().enumerate() {
            if !particle.position.iter().all(|&x| x.is_finite()) {
                return Err(PhysicsError::LagrangeError(
                    format!("Particle {i} has non-finite position")
                ));
            }
            
            if particle.mass <= 0.0 || !particle.mass.is_finite() {
                return Err(PhysicsError::LagrangeError(
                    format!("Particle {i} has invalid mass: {}", particle.mass)
                ));
            }
        }
    ...
    pub fn find_all_lagrange_points(&mut self, particles: &[PrimeParticle]) -> PhysicsResult<Vec<LagrangePoint>> {
    ...
    fn find_lagrange_points_for_pair(
        ...
        let l1 = LagrangePoint {
            ...
            nearest_particle_distance: self.calculate_nearest_distance(&l1_position, particle1, particle2),
        ...
        let l2 = LagrangePoint {
            ...
            clustered_primes: Vec::new(),
            nearest_particle_distance: self.calculate_nearest_distance(&l2_position, particle1, particle2),
        ...
        let l3 = LagrangePoint {
            ...
            clustered_primes: Vec::new(),
            nearest_particle_distance: self.calculate_nearest_distance(&l3_position, particle1, particle2),
        ...
        let (l4_position, l5_position) = self.calculate_triangular_positions(particle1, particle2)?;
        ...
        let l4 = LagrangePoint {
            ...
            clustered_primes: Vec::new(),
            nearest_particle_distance: self.calculate_nearest_distance(&l4_position, particle1, particle2),
        ...
        let l5 = LagrangePoint {
            ...
            clustered_primes: Vec::new(),
            nearest_particle_distance: self.calculate_nearest_distance(&l5_position, particle1, particle2),
    ...
    fn analyze_clustering_at_point_static(
        ...
        for i in 0..prime_count.min(5) {
            ...
            lagrange_point.clustered_primes.push(BigUint::from(base_value));
        }
