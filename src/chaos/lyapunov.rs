...
impl LyapunovCalculator {
    ...
    pub fn calculate_lyapunov(&self, total_time: f64) -> f64 {
        ...
        let sum_log_growth: f64 = self.renorm_factors.iter().map(|&factor| factor.ln()).sum();
...
impl ChaosMap {
    ...
    pub fn measure_configuration_chaos(
        ...
        // Store result
        self.chaos_levels
            .insert((outer, inner, k_outer, k_inner), lyapunov);
        ...
        for outer in [1, 3, 7, 9] {
            for inner in [1, 3, 5, 7, 9] {
                for k_outer in 0..3 {
                    for k_inner in 0..3 {
                        let lambda =
                            self.measure_configuration_chaos(outer, inner, k_outer, k_inner);
