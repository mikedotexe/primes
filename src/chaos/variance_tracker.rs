    ...
    pub fn analyze(&mut self) -> PhysicsResult<VarianceReport> {
        ...
        for seed in self.seed_range.clone() {
            ...
            for _ in 0..self.runs_per_seed {
                let builder = crate::membrane::MembraneBuilder::new(self.config.clone())
                    .with_seed(seed);
        ...
        let total_successes: usize = self.results.values()
            .map(|(s, _)| *s)
            .sum();
        ...
        let seed_rates: Vec<f64> = self.results.values()
            .map(|(s, _)| *s as f64 / self.runs_per_seed as f64)
            .collect();
        
        // Calculate variance
        let mean_rate = seed_rates.iter().sum::<f64>() / seed_rates.len() as f64;
        let variance = seed_rates.iter()
            .map(|r| (r - mean_rate).powi(2))
            .sum::<f64>() / seed_rates.len() as f64;
        
        let std_dev = variance.sqrt();
        
        // Find best and worst seeds
        let mut seed_performance: Vec<(u8, f64)> = self.results.iter()
            .map(|(seed, (successes, _))| {
                (*seed, *successes as f64 / self.runs_per_seed as f64)
            })
            .collect();
        seed_performance.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
        
        let best_seeds: Vec<(u8, f64)> = seed_performance.iter()
            .take(3)
            .cloned()
            .collect();
        
        let worst_seeds: Vec<(u8, f64)> = seed_performance.iter()
            .rev()
            .take(3)
            .cloned()
            .collect();
...
impl VarianceReport {
    ...
    pub fn format_report(&self) -> String {
        ...
        report.push_str(&format!("  Successful generations: {} ({:.1}%)\n", 
            self.total_successes, self.success_rate * 100.0));
        report.push_str(&format!("  Unique primes found: {}\n", self.unique_primes_count));
        report.push_str(&format!("  Prime diversity index: {:.3}\n", self.prime_diversity));
        report.push('\n');
        report.push_str("Seed Performance:\n");
        report.push_str(&format!("  Mean success rate: {:.1}%\n", self.mean_success_rate * 100.0));
        report.push_str(&format!("  Standard deviation: {:.1}%\n", self.std_deviation * 100.0));
        report.push('\n');
        report.push_str("  Best seeds:\n");
        for (seed, rate) in &self.best_seeds {
            report.push_str(&format!("    Seed {}: {:.1}% success\n", seed, rate * 100.0));
        }
        report.push('\n');
