...
use crate::gravity::{PrimeParticle, ForceCalculator};
...
impl NBodyLyapunov {
    ...
    pub fn step(&mut self) -> PhysicsResult<()> {
        ...
        let forces = self.force_calc.calculate_all_forces(
            &self.particles, 
            &self.constants, 
            self.time
        )?;
        ...
        self.integrator.step(&mut self.particles, &forces, self.dt, self.time)?;
        self.shadow_integrator.step(&mut self.shadow_particles, &shadow_forces, self.dt, self.time)?;
        ...
        self.energy_history.push((self.time, orig_energy, shadow_energy));
    ...
    pub fn calculate_lyapunov(&self) -> f64 {
        ...
        let sum_log_growth: f64 = self.renorm_events.iter()
        ...
        for i in 0..particles.len() {
            for j in i+1..particles.len() {
                ...
                if dist > self.constants.min_separation {
                    potential -= self.constants.gravity_constant * 
                        particles[i].mass * particles[j].mass / dist;
                }
            }
        }
    ...
    pub fn calculate_all_forces(&mut self, particles: &[PrimeParticle]) -> PhysicsResult<Vec<[f64; 2]>> {
        self.force_calc.calculate_all_forces(particles, &self.constants, self.time)
    }
...
impl ConfigurationChaosAnalyzer {
    ...
    pub fn analyze_configuration(
        ...
        let energy_drift = if let (Some((_, e0, _)), Some((_, ef, _))) = 
            (lyapunov_calc.energy_history.first(), lyapunov_calc.energy_history.last()) {
        ...
        // Find max velocity
        let max_velocity = lyapunov_calc.particles.iter()
            .map(|p| p.speed())
            .fold(0.0, f64::max);
        ...
        self.chaos_prime_correlation.insert((outer, inner, k_outer, k_inner), metrics.clone());
    ...
    pub fn find_optimal_chaos(&self) -> Vec<(ChaosConfig, &ChaosPrimeMetrics)> {
        let mut results: Vec<_> = self.chaos_prime_correlation.iter()
            .map(|(config, metrics)| (*config, metrics))
            .collect();
        
        // Sort by prime success rate
        results.sort_by(|a, b| b.1.prime_success_rate.partial_cmp(&a.1.prime_success_rate).unwrap());
    ...
    pub fn test_moderate_chaos_hypothesis(&self) -> bool {
        ...
        println!("  Stable configs (λ < 0.05): {:.1}% prime success", avg_stable * 100.0);
        println!("  Moderate chaos (0.05 ≤ λ < 0.2): {:.1}% prime success", avg_moderate * 100.0);
        println!("  Extreme chaos (λ ≥ 0.2): {:.1}% prime success", avg_extreme * 100.0);
