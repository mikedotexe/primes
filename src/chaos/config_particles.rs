...
use crate::membrane::{MembraneConfig, MembraneBuilder};
...
fn generate_characteristic_particles(
    ...
    let particles = characteristic_primes.into_iter()
        .take(2)
        .enumerate()
        .map(|(i, prime_value)| {
            let position = if i == 0 {
...
fn get_characteristic_primes(config: &MembraneConfig) -> Vec<BigUint> {
    ...
    let primes: Vec<u64> = match (config.base, config.outer, config.inner, config.k_outer, config.k_inner) {
        ...
        (10, _, _, _, _) if matches!(&config.construction_type, 
...
pub fn generate_chaos_test_particles(
    ...
    let shadow_particles = main_particles.iter()
        .map(|p| {
