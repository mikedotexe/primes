...
use super::{Integrator, IntegrationStats};
...
impl Integrator for SymplecticIntegrator {
    fn step(
        ...
        let accelerations: Vec<[f64; 2]> = particles.iter()
            .zip(forces.iter())
    ...
    fn test_symplectic_energy_conservation() {
        ...
        for i in 0..100 {
            ...
            let r = ((particles[0].position[0] - particles[1].position[0]).powi(2) +
                     (particles[0].position[1] - particles[1].position[1]).powi(2)).sqrt();
            let f = 1.0 / (r * r);
            ...
            integrator.step(&mut particles, &forces, 0.01, i as f64 * 0.01).unwrap();
        }
