use super::{ExplainMap, JoinedGrid};
...
pub fn verify_to_csv<P: AsRef<Path>>(
    ...
    if let Some(dir) = out_csv.as_ref().parent() {
        if !dir.exists() {
    ...
    writeln!(
        w,
        "base,mid_len,inner_zero,obs,pred,ci_width,delta_abs,enrichment,union_any,top_moduli"
    )?;
    for p in &grid.pairs {
        let pred = p.pred_local_exact.or(p.pred_local);
        let obs = p.obs_density;
        let ciw = match (p.ci_lo, p.ci_hi) {
            (Some(a), Some(b)) => Some((b - a).abs()),
            _ => None,
        };
        let dabs = match (obs, pred) {
            (Some(o), Some(pr)) => Some(o - pr),
            _ => None,
        };
        let enr = match (obs, pred) {
            (Some(o), Some(pr)) => Some(super::enrichment(o, pr)),
            _ => None,
        };
