...
use crate::prime_lut::{SIGNATURES, SigRow as LutSigRow};
use std::mem;
...
impl GpuSieve {
        ...
        let device = Device::system_default()
            .ok_or_else(|| {
                "No Metal GPU available. Ensure you're running on macOS 10.11+ \
                 with a Metal-capable GPU (all Macs since 2012).".to_string()
            })?;
    ...
    pub fn sieve(&self, candidates: &[u32], _base: u32) -> Result<Vec<u32>, String> {
        ...
        let sig_rows: Vec<SigRow> = SIGNATURES
            .iter()
            .take(100)
            .map(|LutSigRow { s, g, p, .. }| SigRow { s: *s, g: *g, p: *p })
            .collect();
...
impl GpuSieve {
    ...
    pub fn new() -> Result<Self, String> { 
        Err("GPU acceleration requires macOS with Metal support. \
             Please use CPU-based implementations on this platform.".into()) 
    }
    
    /// Always returns an error on non-macOS platforms
    pub fn sieve(&self, _: &[u32], _: u32) -> Result<Vec<u32>, String> {
        Err("GPU acceleration requires macOS with Metal support. \
             This method should not be called on non-macOS platforms.".into())
    }
}