...
use super::{ForceResult, InteractionType, PrimeParticle};
...
impl ForceCalculator {
    ...
    pub fn calculate_all_forces(
        ...
        for i in 0..particles.len() {
            for j in i + 1..particles.len() {
                let force_result =
                    self.calculate_pairwise_force(&particles[i], &particles[j], constants, i, j)?;
    ...
    pub fn calculate_pairwise_force(
        ...
        // Combine forces
        let total_magnitude =
            gravitational_force + charge_force + base_metric_force + membrane_force;
        let total_force = [total_magnitude * unit_x, total_magnitude * unit_y];
    ...
    fn calculate_membrane_resonance_force(
        ...
        resonance_score * p1.physics_cache.resonance_factor * p2.physics_cache.resonance_factor
            / (distance * distance)
    }
    ...
    pub fn get_force_statistics(&self) -> ForceStatistics {
        ...
        if stats.total_calculations > 0 {
            stats.average_force_magnitude =
                stats.total_force_magnitude / stats.total_calculations as f64;
        }
...
impl ForceStatistics {
    ...
    pub fn dominant_interaction(&self) -> InteractionType {
        let interactions = [
            ...
            (
                self.quantum_exchange_dominant,
                InteractionType::QuantumExchange,
            ),
        ];
