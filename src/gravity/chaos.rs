...
impl ChaosDetector {
    ...
    pub fn calculate_chaos_indicator(&mut self, particles: &[PrimeParticle], current_time: f64) -> PhysicsResult<f64> {
        if particles.len() < 2 {
            return Ok(0.0);
        }
        ...
        for i in 0..particles.len() {
            for j in i+1..particles.len() {
                let divergence = self.calculate_velocity_divergence(&particles[i], &particles[j])?;
    ...
    fn calculate_velocity_divergence(&self, p1: &PrimeParticle, p2: &PrimeParticle) -> PhysicsResult<f64> {
    ...
    fn update_lyapunov_estimate(&mut self, _current_time: f64) {
        ...
        let recent_chaos: Vec<f64> = self.chaos_history
            .iter()
            .rev()
            .take(recent_window)
            .cloned()
