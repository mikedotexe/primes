...
impl FlexibleAnalysis {
    ...
    pub fn analyze(membrane: &str) -> PhysicsResult<Self> {
        ...
        if digits.is_empty() {
            return Err(PhysicsError::InvalidMembrane("No digits found".to_string()));
        }
    ...
    fn detect_structure(chars: &[char], zero_runs: &[usize]) -> HashMap<&'static str, Vec<usize>> {
        ...
        if !digit_positions.is_empty() {
            map.insert(
                ...
                vec![
                    digit_positions[0],
                    digit_positions[digit_positions.len() - 1],
                ],
            );
        }
    ...
    pub fn total_resonance(&self) -> f64 {
        let pattern_score: f64 = self.resonance_patterns.iter().map(|p| p.strength).sum();
...
pub fn construct_general_membrane(components: Vec<MembraneComponent>) -> PhysicsResult<String> {
    ...
    for component in components {
        match component {
            MembraneComponent::Digit(d) => {
                if d >= 10 {
                    return Err(PhysicsError::InvalidMembrane(format!(
                        "Digit {d} must be 0-9"
                    )));
                }
