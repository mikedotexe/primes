...
use super::{MembraneConfig, OptimizationTarget, construct_symmetric_membrane};
...
fn get_adaptive_parameters(base: u32, target: &OptimizationTarget) -> (u32, u32, u32, u32) {
    match (base, target) {
        ...
        (10, OptimizationTarget::PatternMatching(patterns)) => {
            if patterns.iter().any(|p| p.contains("37") || p.contains("73")) {
...
pub fn generate_adaptive_configs(base: u32) -> Vec<AdaptiveAnalysis> {
    ...
    configs.sort_by(|a, b| b.adaptation_score().partial_cmp(&a.adaptation_score()).unwrap());
...
mod tests {
    ...
    fn test_edge_pair_detection() {
        let analysis = AdaptiveAnalysis::new(
            10, 
            (3, 7, 2, 2), 
            OptimizationTarget::MaxDensity
        );
