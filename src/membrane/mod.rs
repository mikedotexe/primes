...
use crate::{is_prime, PhysicsError, PhysicsResult};
...
impl MembraneConfig {
    ...
    pub fn quantum(base: u32, orbital: OrbitalType) -> Self {
        let (outer, inner, k_outer, k_inner, quantum_numbers) = get_quantum_config(base, &orbital);
...
impl MembraneBuilder {
    ...
    pub fn build(self) -> PhysicsResult<PrimeParticle> {
        ...
        for attempt in 0..self.max_attempts {
            ...
            let middle = generate_middle_content(&self.config, &mut rng, effective_attempt)?;

            // Construct the membrane
            let membrane_number = construct_membrane_number(&self.config, &middle)?;
...
fn generate_palindrome_middle(
    ...
    let start_idx = if config.middle_length.is_multiple_of(2) {
        half_len
    } else {
        half_len - 1
    };
...
fn generate_37_pattern_middle(
    ...
    for i in 0..config.middle_length {
        let digit = if rng.gen_bool(0.3) {
            // 30% chance for 3 or 7
            if rng.gen_bool(0.5) {
                3
            } else {
                7
...
fn construct_membrane_number(config: &MembraneConfig, middle: &str) -> PhysicsResult<String> {
    match &config.construction_type {
        ConstructionType::Symmetric => symmetric::construct_symmetric_membrane(
            config.outer,
            config.inner,
            middle,
            config.k_outer,
        ...
        ConstructionType::Adaptive {
            optimization_target,
        } => adaptive::construct_adaptive_membrane(config, middle, optimization_target),
        ConstructionType::Quantum {
            orbital_type,
            quantum_numbers,
        } => quantum::construct_quantum_membrane(config, middle, orbital_type, quantum_numbers),
    }
}
...
fn estimate_density(base: u32, outer: u32, inner: u32, k_outer: u32, k_inner: u32) -> f64 {
    ...
    let k_penalty = if k_outer == 0 && k_inner == 0 {
        1.0
    } else {
        0.6
    };
