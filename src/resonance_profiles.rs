...
impl ConfigurationProfile {
    ...
    pub fn record_success(&mut self, seed: u8, prime_length: usize) {
        ...
        self.avg_prime_length = (self.avg_prime_length * (n - 1.0) + prime_length as f64) / n;
    }
...
impl BaseResonanceProfile {
    ...
    pub fn discover_resonances(
                            ...
                            for length in 1..=max_middle_length {
                                ...
                                match construct_symmetric_membrane(
                                    ...
                                    Ok(membrane_str) => {
                                        ...
                                        if let Ok(num) =
                                            BigUint::from_str_radix(&membrane_str, self.base)
                                        {
                                            if is_prime(&num) {
        ...
        self.configurations = config_map.into_values().collect();
    ...
    fn build_seed_map(&mut self) {
        ...
        for config in &self.configurations {
            for &seed in &config.successful_seeds {
                self.seed_map
                    .add_configuration(seed, &config.config, config.is_exclusive);
            }
        }
    }
    ...
    fn calculate_statistics(&mut self) {
        self.stats = ProfileStatistics {
            ...
            exclusive_configurations: self
                .configurations
                .iter()
                .filter(|c| c.is_exclusive)
                .count(),
            max_success_rate: self
                .configurations
                .first()
            ...
            avg_success_rate: if self.configurations.is_empty() {
                ...
                self.configurations
                    ...
                    .map(|c| c.success_rate)
                    .sum::<f64>()
                    / self.configurations.len() as f64
            },
...
impl SeedResonanceMap {
    ...
    pub fn add_configuration(&mut self, seed: u8, config: &MembraneConfig, is_exclusive: bool) {
        self.map.entry(seed).or_default().push(SeedConfigEntry {
            config: config.clone(),
            is_exclusive,
        });
    }
...
impl fmt::Display for BaseResonanceProfile {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(
            ...
            self.stats.exclusive_configurations
        )?;
        writeln!(
            f,
            "Max Success Rate: {:.2}%",
            self.stats.max_success_rate * 100.0
        )?;
...
impl ResonanceAnalyzer {
    ...
    pub fn universal_configurations(&self) -> Vec<MembraneConfig> {
        ...
        for config_profile in &first_base.configurations {
            ...
            let is_universal = self
                .profiles
                .values()
                .all(|profile| profile.configurations.iter().any(|cp| cp.config == *config));
    ...
    pub fn exclusive_comparison(&self) -> HashMap<u32, Vec<(MembraneConfig, u8)>> {
        self.profiles
            .iter()
            .map(|(&base, profile)| {
                let exclusives = profile
                    .exclusive_configurations()
                    .into_iter()
                    .filter_map(|cp| cp.exclusive_seed().map(|seed| (cp.config.clone(), seed)))
                    .collect();
                (base, exclusives)
            })
            .collect()
