...
use crate::membrane::{ConstructionType, MembraneConfig};
...
impl FailureAnalyzer {
    ...
    fn diagnose_complete_failure(&self, config: &MembraneConfig) -> String {
        ...
        if config.base == 10 && config.outer == 5 {
            return "Digit 5 as boundary in base 10 creates wave node (center point), not antinode"
                .to_string();
        }

        // Powers of 2 in even bases
        if config.base.is_multiple_of(2) {
            let powers_of_2 = [2, 4, 8, 16];
            if powers_of_2.contains(&config.outer) || powers_of_2.contains(&config.inner) {
                return format!(
                    "Power of 2 as boundary in even base {} creates systematic divisibility",
                    config.base
                );
            }
        }
        ...
        if config.k_outer > 10 || config.k_inner > 10 {
            return "Excessive zero padding (k > 10) dilutes prime density below detection"
                .to_string();
        }
    ...
    fn check_digit_exclusion(&self, results: &[(BigUint, bool)]) -> Option<FailureMode> {
        ...
        let working_digits: Vec<u32> = digit_success
            .iter()
            .filter(|(_, (success, total))| *success > 0 && *total > 0)
            .map(|(digit, _)| *digit)
            .collect();

        let failed_digits: Vec<u32> = digit_success
            .iter()
            .filter(|(_, (success, total))| *success == 0 && *total > 0)
            .map(|(digit, _)| *digit)
            .collect();
    ...
    fn check_structural_defects(&self, config: &MembraneConfig) -> Option<FailureMode> {
        ...
        if let ConstructionType::Breathing {
            left_k_outer,
            left_k_inner,
            right_k_outer,
            right_k_inner,
        } = config.construction_type
        {
            let asymmetry = ((left_k_outer as i32 - right_k_outer as i32).abs()
                + (left_k_inner as i32 - right_k_inner as i32).abs())
                as f64;
            ...
            if asymmetry > 5.0 {
                return Some(FailureMode::StructuralDefect {
                    ...
                    description: format!(
                        "Left/right imbalance of {asymmetry} breaks wave symmetry"
                    ),
                });
            }
        }
        ...
        if self.is_incompatible_pair(config.outer, config.inner, config.base) {
            return Some(FailureMode::StructuralDefect {
                ...
                description: format!(
                    "Digits {} and {} create destructive interference in base {}",
    ...
    fn is_incompatible_pair(&self, digit1: u32, digit2: u32, base: u32) -> bool {
        ...
        if base.is_multiple_of(2) && digit1.is_multiple_of(2) && digit2.is_multiple_of(2) {
            return true;
        }

        // Both multiples of 5 in base 10
        if base == 10 && digit1.is_multiple_of(5) && digit2.is_multiple_of(5) {
            return true;
        }
    ...
    fn extract_lessons(
        ...
        match failure_mode {
            FailureMode::CompleteFailure { reason } => {
                ...
                lessons
                    .push("This configuration creates systematic composite generation".to_string());
            }

            FailureMode::DigitExclusion {
                working_digits,
                failed_digits,
            } => {
                lessons.push(format!(
                    "Only digits {working_digits:?} work in this configuration"
                ));
                lessons.push(format!(
                    "Digits {failed_digits:?} are systematically excluded"
                ));
                lessons.push(
                    "Configuration creates selective resonance affecting specific digits"
                        .to_string(),
                );
            }

            FailureMode::BelowRandom {
                actual_rate,
                expected_random,
            } => {
                lessons.push(format!(
                    "Performance {:.2}% vs random {:.4}% suggests active suppression",
                    actual_rate * 100.0,
            ...
            FailureMode::StructuralDefect {
                ...
                if let Some(&structure_pct) = params.get("structure_percentage") {
                    if structure_pct > 90.0 {
                        lessons.push(
                            "Excessive structure (>90%) enters volatile resonance regime"
                                .to_string(),
                        );
                    }
                }
        ...
        match failure_mode {
            ...
            FailureMode::DigitExclusion { working_digits, .. } => {
                suggestions.push(format!(
                    "Focus on configurations that emphasize digits {working_digits:?}"
                ));
            ...
            FailureMode::StructuralDefect { defect_type, .. } => {
                if defect_type.contains("Asymmetry") {
                    ...
                    suggestions
                        .push("Or explore controlled asymmetry with k-difference ≤ 2".to_string());
                }
            }
    ...
    fn update_patterns(&mut self, analysis: &FailureAnalysis) {
        ...
        *self.failure_patterns.entry(mode_key).or_insert(0) += 1;
        ...
        let key = format!(
            "outer_{}_base_{}",
            analysis.config.outer, analysis.config.base
        );
        *self.failure_patterns.entry(key).or_insert(0) += 1;

        let key = format!(
            "inner_{}_base_{}",
            analysis.config.inner, analysis.config.base
        );
        *self.failure_patterns.entry(key).or_insert(0) += 1;
    }
    ...
    pub fn generate_failure_report(&self) -> String {
        ...
        for (mode, count) in mode_counts {
            report.push_str(&format!(
                ...
                mode,
                count,
                count as f64 / self.failure_database.len() as f64 * 100.0
            ));
        }
        ...
        let all_lessons: Vec<String> = self
            .failure_database
            .iter()
