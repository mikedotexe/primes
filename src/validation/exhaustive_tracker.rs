...
impl ExhaustiveTracker {
    ...
    pub fn generate_report(&self) -> String {
        ...
        report.push_str(&format!(
            "Configurations Selected: {} ({:.1}%)\n",
            selected,
            selected as f64 / self.all_tests.len() as f64 * 100.0
        ));
        ...
        param_list.sort_by(|a, b| {
            b.1.average_success_rate
                .partial_cmp(&a.1.average_success_rate)
                .unwrap()
        });
        ...
        let selected_avg: f64 = self
            ...
            .sum::<f64>()
            / selected as f64;
        let unselected_avg: f64 = self
            .all_tests
            .iter()
            .filter(|t| !t.selected)
            .map(|t| t.success_rate)
            .sum::<f64>()
            / (self.all_tests.len() - selected) as f64;

        report.push_str(&format!(
            "  Selected configs avg: {:.4}%\n",
            selected_avg * 100.0
        ));
        report.push_str(&format!(
            "  Unselected configs avg: {:.4}%\n",
