...
use crate::validation::{ConfigResult, ValidationContext, ValidationResult};
...
impl<'a> RandomBaseline<'a> {
    ...
    pub fn compare_with_membrane(
        ...
        let random_results = self.generate_random_baseline(config, middle_digits.len(), strategy);

        // Calculate statistics
        let validation =
            self.calculate_validation_stats(&membrane_results, &random_results, config);
    ...
    fn generate_random_baseline(
        ...
        for _ in 0..count {
            let num = match strategy {
                RandomStrategy::UniformDigits => self.random_uniform_digits(config.total_digits()),
                RandomStrategy::StructurePreserving => self.random_structure_preserving(config),
                RandomStrategy::PrimeBiased => self.random_prime_biased(config.total_digits()),
                RandomStrategy::UniformBits => self.random_uniform_bits(config.total_digits()),
            };
    ...
    fn random_structure_preserving(&mut self, config: &MembraneConfig) -> BigUint {
        ...
        let temp_config =
            MembraneConfig::new(config.base, outer, inner, config.k_outer, config.k_inner);

        temp_config
            .construct_number(middle)
    ...
    fn calculate_validation_stats(
        ...
        let ci =
            self.bootstrap_confidence_interval(membrane_results, self.context.bootstrap_iterations);
        ...
        let example_primes: Vec<BigUint> = membrane_results
            .iter()
        ...
        ValidationResult {
            ...
            improvement_factor: if random_rate > 0.0 {
                method_rate / random_rate
            } else {
                f64::INFINITY
            },
            p_value,
    ...
    fn bootstrap_confidence_interval(
        ...
        (
            bootstrap_rates[lower_idx],
            bootstrap_rates[upper_idx.min(iterations - 1)],
        )
    }
    ...
    fn print_validation_report(&self, result: &ValidationResult, elapsed: f64) {
        ...
        println!(
            "  Improvement:   {:.1}x better than random",
