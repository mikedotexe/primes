...
impl TidalAnalyzer {
        ...
        for i in 0..rows {
            for j in 0..cols {
                ...
                if let TidalZone::Organizing { .. } = &field[i][j].zone_type {
                    ...
                    let zone =
                        self.explore_organization_zone(field, particles, i, j, &mut visited)?;
    ...
    pub fn take_snapshot(
        ...
        let total_organized = zones.iter().map(|z| z.captured_primes.len()).sum();

        let field_strength = field
            .iter()
    ...
    pub fn analyze_evolution(&self) -> TidalEvolution {
        ...
        let organization_growth =
            (last.total_organized_primes as f64 / first.total_organized_primes.max(1) as f64) - 1.0;
    ...
    fn count_pattern_changes(&self) -> usize {
        ...
        for i in 1..self.tidal_history.len() {
            ...
            if (prev.organization_zones.len() as i32 - curr.organization_zones.len() as i32).abs()
                > 1
            {
                changes += 1;
            }
...
impl RocheCalculator {
    ...
    pub fn calculate_roche_limit(&self, primary: &PrimeParticle, secondary: &PrimeParticle) -> f64 {
        // Classical Roche limit adapted for prime physics
        let density_ratio = primary.mass / secondary.mass;
        let base_limit = self.coefficient
            * secondary.physics_cache.effective_radius
