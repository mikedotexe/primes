...
use super::{OptimizationStrategy, SystemContext, Workload, Metrics, Feedback, Priority};
...
impl OptimizationStrategy for Wheel30Strategy {
    ...
    fn apply(&self, workload: &mut Workload, context: &SystemContext) -> Result<Metrics, PhysicsError> {
        ...
        // Update workload
        workload.memory_footprint = (workload.memory_footprint as f64 * (1.0 - memory_reduction)) as usize;
        workload.expected_duration = Duration::from_secs_f64(
            workload.expected_duration.as_secs_f64() / throughput_boost
        );
    ...
    fn learn(&mut self, feedback: &Feedback) {
        ...
        self.effectiveness_score.store(new_score.to_bits(), Ordering::Relaxed);
    }
...
impl OptimizationStrategy for DVFSAdaptiveStrategy {
    ...
    fn apply(&self, workload: &mut Workload, context: &SystemContext) -> Result<Metrics, PhysicsError> {
        ...
        self.optimal_segment_size.store(optimal_size as u64, Ordering::Relaxed);
        ...
        // Update workload
        workload.expected_duration = Duration::from_secs_f64(
            workload.expected_duration.as_secs_f64() / efficiency_gain
        );
...
impl OptimizationStrategy for CacheOptimizedStrategy {
    ...
    fn apply(&self, workload: &mut Workload, _context: &SystemContext) -> Result<Metrics, PhysicsError> {
        // Cache optimization provides consistent moderate improvements
        let improvement = 1.12;
        
        workload.expected_duration = Duration::from_secs_f64(
            workload.expected_duration.as_secs_f64() / improvement
        );
...
impl OptimizationStrategy for SIMDStrategy {
    ...
    fn apply(&self, workload: &mut Workload, context: &SystemContext) -> Result<Metrics, PhysicsError> {
        ...
        workload.expected_duration = Duration::from_secs_f64(
            workload.expected_duration.as_secs_f64() / speedup
        );
    ...
    fn learn(&mut self, feedback: &Feedback) {
        ...
        if feedback.success {
            let current = f64::from_bits(self.effectiveness_score.load(Ordering::Relaxed));
            if current < 0.95 {
                self.effectiveness_score.store((current + 0.01).to_bits(), Ordering::Relaxed);
            }
        }
    }
...
mod tests {
    use super::*;
    use crate::optimization::{CpuInfo, Architecture, CacheSizes, Priority};
