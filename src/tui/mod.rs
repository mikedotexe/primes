...
impl LagrangeUIState {
    ...
    pub fn cycle_configuration(&mut self) {
        let configs = vec![
            ...
            (MembraneConfig::new(10, 3, 7, 1, 1), "(3,7) k=(1,1) base 10 - Exclusive!"),
            (MembraneConfig::new(10, 7, 7, 1, 1), "(7,7) k=(1,1) base 10"),
            (MembraneConfig::new(10, 3, 3, 0, 1), "(3,3) k=(0,1) base 10 - Breathing"),
            (MembraneConfig::new(6, 1, 5, 0, 0), "(1,5) k=(0,0) base 6 - Champion"),
        ];
        
        let current_idx = configs.iter().position(|(c, _)| {
...
fn format_visual(value: &BigUint, _config: &MembraneConfig) -> String {
    ...
    let visual_chars: Vec<String> = chars.iter().enumerate().map(|(i, &c)| {
...
pub fn render_to_text(state: &LagrangeUIState) -> String {
    ...
    output.push('\n');
    output.push_str(&format!("{:^148}", "⚛️  Lagrange Point Explorer - Prime Atomic Interactions"));
    output.push('\n');
    output.push_str(&"─".repeat(148));
    output.push('\n');
    
    // Status
    let config_str = format!("({},{}) k=({},{}) b{}", 
        state.config.outer, state.config.inner,
        state.config.k_outer, state.config.k_inner,
        state.config.base
    );
