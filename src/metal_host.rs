...
impl MetalCtx {
    ...
    pub fn new() -> Result<Self, String> {
        ...
        let device = Device::system_default()
            .ok_or_else(|| "No Metal GPU found. Metal requires macOS 10.11+ with a Metal-capable GPU.".to_string())?;
        ...
        let lib = device
            ...
            .map_err(|e| format!("Failed to load Metal shader library: {e}. Ensure shaders are compiled."))?;
        
        // Get the sieve kernel function
        let func = lib
            .get_function("sieve_affine", None)
            .map_err(|_| "Kernel 'sieve_affine' not found in shader library. Check shader compilation.".to_string())?;
    ...
    pub fn run(&self, ctab: &[u32], sig: &[SigRow]) -> Vec<u32> {
        ...
        unsafe {
            std::slice::from_raw_parts(out_buf.contents() as *const u32, mask_words).to_vec()
        }
    }
}
