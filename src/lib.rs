...
pub use performance::{PerfMetrics, PerfMonitor};
pub use prime_sieve::{BitSieve, WarmResult};
pub use resonance_profiles::{
    BaseResonanceProfile, ConfigurationProfile, ResonanceAnalyzer, SeedResonanceMap,
};
...
impl PrimeUniverse {
    ...
    pub fn step(&mut self) -> PhysicsResult<()> {
        ...
        if self.dt <= 0.0 || !self.dt.is_finite() {
            return Err(PhysicsError::IntegrationError(format!(
                "Invalid time step: {}",
                self.dt
            )));
        }
        ...
        let forces = self
            .field
            .calculate_forces(&self.particles, &self.constants)?;

        // Validate forces vector length
        if forces.len() != self.particles.len() {
            return Err(PhysicsError::IntegrationError(format!(
                "Forces vector length {} doesn't match particles length {}",
                forces.len(),
                self.particles.len()
            )));
        }
    ...
    pub fn analyze_tidal_field(
        ...
        let field = self
            .tidal_analyzer
            .calculate_tidal_field(&self.particles, grid_size)?;

        // Find organization zones
        let zones = self
            .tidal_analyzer
            .find_organization_zones(&self.particles, &field)?;

        // Take a snapshot for history
        self.tidal_analyzer
            .take_snapshot(&field, &zones, &self.particles);
    ...
    pub fn tidal_strength_at(&self, point: [f64; 2]) -> PhysicsResult<f64> {
        let field = self
            .tidal_analyzer
            .calculate_tidal_at_point(point, &self.particles)?;
        Ok(field.strength)
    }
    ...
    pub fn discover_base_resonance(
        ...
        profile
            .discover_resonances(outer_range, inner_range, k_range, max_middle_length)
    ...
    pub fn create_trinity(base1: u32, base2: u32, base3: u32) -> PhysicsResult<Self> {
        ...
        let positions = [[1.0, -2.0], [0.0, -6.0], [-5.0, -1.0]];
    ...
    fn integrate_euler(&mut self, forces: Vec<[f64; 2]>) -> PhysicsResult<()> {
        ...
        for (i, particle) in self.particles.iter_mut().enumerate() {
            ...
            if particle.velocity[0].abs() > MAX_VELOCITY
                || particle.velocity[1].abs() > MAX_VELOCITY
            {
                return Err(PhysicsError::IntegrationError(format!(
                    "Velocity too large: ({}, {})",
            ...
            if particle.position[0].abs() > MAX_POSITION
                || particle.position[1].abs() > MAX_POSITION
            {
                return Err(PhysicsError::IntegrationError(format!(
                    "Position too large: ({}, {})",
    ...
    fn update_stability_metrics(&mut self) -> PhysicsResult<()> {
        ...
        for i in 0..self.particles.len() {
            for j in i + 1..self.particles.len() {
                ...
                let r = (dx * dx + dy * dy)
                    .sqrt()
                    .max(self.constants.min_separation);
