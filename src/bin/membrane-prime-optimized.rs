...
fn main() {
    ...
    println!("=====================================");
    println!("Config: base-{}, width={}, boundary=({},{})", args.base, args.width, l, r);
    ...
    let test_start = if args.packed && (args.base == 6 || args.base == 12) {
        ...
        println!("Packed {} candidates into {} u32s in {:?}", 
            candidates.len(), packed.len(), pack_start.elapsed());
    ...
    println!("Test time: {:.2}s ({:.0} candidates/sec)", elapsed.as_secs_f64(), throughput);
    ...
    if args.packed {
        println!("  Nibble packing reduced memory by {:.1}x", 
    ...
    // GPU projection
    let gpu_speedup = if args.base == 6 { 25 } else if args.base == 12 { 30 } else { 20 };
    let gpu_throughput = throughput * gpu_speedup as f64;
    println!("\n🎯 GPU Projection:");
    println!("  Expected throughput: {:.1}M candidates/sec", gpu_throughput / 1_000_000.0);
    println!("  Primes/hour: {:.1}B", gpu_throughput * density * 3600.0 / 1_000_000_000.0);
}
