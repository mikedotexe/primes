...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Parse zero padding
    let zeros: Vec<u32> = args.zeros.split(',')
    ...
    if let Some(ref mut ledger) = ledger {
        ...
        writeln!(ledger, "EVT CONFIG base={} w={} L={} R={} μ={}", 
    ...
    if args.output == "text" {
        ...
        println!("Base: {}, Width: {}, Boundary: ({},{}), Padding: ({},{})", 
            args.base, args.width, l, r, r1, r2);
    ...
    match args.output.as_str() {
        "text" => {
            println!("Found {} primes in {:.2}s", primes.len(), elapsed.as_secs_f64());
        ...
        "evtlog" => {
            if let Some(ref mut ledger) = ledger {
                writeln!(ledger, "EVT STATS t={} tested={} primes={} density={:.4} throughput={:.0}",
                    Local::now().format("%Y-%m-%dT%H:%MZ"),
                    args.count, primes.len(), density, throughput)?;
                
                for (i, (seed, prime)) in primes.iter().enumerate().take(10) {
                    writeln!(ledger, "EVT PRIME t={} idx={} seed={} value={}",
                        Local::now().format("%Y-%m-%dT%H:%MZ"),
                        i, seed, prime)?;
                    
                    if args.watermark {
                        let k = 3 + (seed % 10);
                        writeln!(ledger, "EVT WATERMARK seed={} k={} amplitude=1.0",
                            seed, k)?;
                    }
                }
            }
