    ...
    if has_metal_feature && is_macos {
        ...
        if !shader_path.exists() {
            ...
            for metal_file in metal_files {
                let file_stem = PathBuf::from(metal_file).file_stem().unwrap().to_str().unwrap().to_string();
            ...
            let metallib_status = metallib_cmd.status()
        ...
        println!("cargo:rustc-env=METALLIB_PATH={}", metallib_output.display());
        ...
        if metallib_optimized.exists() {
            println!("cargo:rustc-env=METALLIB_OPTIMIZED_PATH={}", metallib_optimized.display());
        } else {
            // Set to regular metallib as fallback
            println!("cargo:rustc-env=METALLIB_OPTIMIZED_PATH={}", metallib_output.display());
        }
    }
}