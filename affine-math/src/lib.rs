//! Affine arithmetic core: sound first-order enclosures with explicit noise symbols ε∈[-1,1].
//!
//! Invariant: `terms` sorted by symbol, merged (no duplicates).
//! `to_interval` = `[a0 - Σ|ai|, a0 + Σ|ai|]`.
//! Multiplication: `z0 = a0*b0`, shared-symbol coeffs `a0*b_i + b0*a_i`,
//! remainder `ρ = (Σ|a_i|)*(Σ|b_j|)` on a fresh ε. Float roundoff is absorbed into `ρ`.
//!
//! Example:
//! ```
...
/// Symbol allocation context. Own a context to preserve determinism.
...
/// Affine form: `a0 + Σ (coeff[sym] * ε_sym)`, with `ε∈[-1,1]`.
...
impl Affine {
    ...
    fn _add_term(&mut self, sym: Sym, coeff: f64) {
        if coeff == 0.0 { return; }
    ...
    /// Affine multiply (needs a fresh noise symbol).
    pub fn mul_ctx(&self, other: &Self, ctx: &mut Ctx) -> Self {
        ...
        let rho = {
            ...
            for &(_, ai) in &self.terms { sa = sa + Interval::new(ai.abs(), ai.abs()).unwrap(); }
            let mut sb = Interval::new(0.0, 0.0).unwrap();
            for &(_, bj) in &other.terms { sb = sb + Interval::new(bj.abs(), bj.abs()).unwrap(); }
            (sa * sb).sup()
        };
