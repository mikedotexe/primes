//! Symbol condensation: cap number of terms by keeping the largest coefficients
//! and merging the rest into one new noise term so that the enclosure never shrinks.

use crate::{Affine, Ctx, Sym};

impl Affine {
    /// Condense to at most `max_terms` symbols by preserving the largest `max_terms-1`
    /// absolute coefficients and merging the tail (sum of abs) into a fresh symbol.
    ///
    /// Panics if `max_terms < 1`.
