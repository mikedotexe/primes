...
fn test_validation_with_single_config() {
    ...
    assert!(result.method_success_rate > 0.0, "Should find at least some primes");
    assert!(result.improvement_factor > 1.0, "Should be better than random");
    assert_eq!(result.sample_size, middle_digits.len());
    assert!(result.p_value < 1.0);
    
    println!("Validation test passed:");
    println!("  Method success rate: {:.2}%", result.method_success_rate * 100.0);
    println!("  Random success rate: {:.2}%", result.random_success_rate * 100.0);
...
fn test_exhaustive_tracker() {
    ...
    for config in configs {
        ...
        let reason = if selected { "High performance" } else { "Below threshold" };
...
fn test_failure_analyzer() {
    ...
    let mut results = Vec::new();
    for middle in 0..10 {
        match bad_config.construct_number(middle) {
            Ok(num) => {
                let is_prime = prime_physics_engine::is_prime(&num);
                results.push((num, is_prime));
            }
...
fn test_membrane_builder_integration() {
    ...
    match builder.build() {
        ...
        Err(e) => {
            println!("Failed to generate prime after 100 attempts (expected): {:?}", e);
        }
    }
}
...
fn test_multiple_configs_separately() {
    ...
    for &outer in &outer_values {
        for &inner in &inner_values {
            ...
            // Test with middle = 5
            match config.construct_number(5) {
                Ok(num) => {
    ...
    println!("Tested {} configurations, found {} primes", total_configs, total_primes);
...
fn test_full_validation_pipeline() {
    ...
    for (outer, inner, name) in test_configs {
        ...
        tracker.record_test(
            ...
            10, // mock duration
            selected,
            if selected { "High performance" } else { "Low performance" }
        );
    ...
    // 5. Assertions
    assert!(validation.improvement_factor > 2.0, "Should be significantly better than random");
    assert!(tracker_report.contains("Total Configurations Tested: 5"));
    assert!(!failure_report.is_empty());
    
    let elapsed = start.elapsed();
    println!("\nFull validation pipeline completed in {:.2}s", elapsed.as_secs_f64());
    println!("Best configuration improvement: {:.1}x over random", validation.improvement_factor);
}