...
fn test_fourier_transform_correctness() {
    ...
    println!("  DC component: {:.6} (expected: {:.6})", dc_component, expected_dc);
...
fn test_prime_sequence_harmonic_structure() {
    ...
    for limit in limits {
        ...
        println!("  Dominant frequencies: {}", analysis.dominant_frequencies.len());
    ...
    for size in test_sizes {
        ...
        for seed in 0..5 {
            ...
            let random_seq: Vec<f64> = (0..size)
                .map(|_| if rng.gen::<f64>() < prime_density { 1.0 } else { 0.0 })
                .collect();
        ...
        println!("  Prime harmonic purity: {:.4}", prime_analysis.harmonic_purity);
        println!("  Random purities: {:?}", random_purities.iter().map(|p| format!("{:.4}", p)).collect::<Vec<_>>());
        println!("  Average random purity: {:.4}", avg_random_purity);
        println!("  Ratio (prime/random): {:.2}x", prime_analysis.harmonic_purity / avg_random_purity);
...
fn test_membrane_generated_harmonics() {
    ...
    for (outer, inner, name) in configs {
        ...
        println!("  Generated {} numbers, {} primes ({:.1}% success rate)", 
                 all_numbers.len(), prime_numbers.len(), prime_rate * 100.0);
...
fn test_parseval_theorem() {
    ...
    for (name, signal) in test_signals {
        ...
        assert!(error < 1e-10, "Parseval's theorem violated for {} signal", name);
    }
}
