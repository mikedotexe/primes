...
mod harmonics_tests {
    ...
    fn test_fourier_transform_parseval() {
        ...
        assert!((time_energy - freq_energy).abs() < 1e-10, 
                "Parseval's theorem failed: time={}, freq={}", time_energy, freq_energy);
    }
    ...
    fn test_dominant_frequencies_threshold() {
        ...
        assert!(freq == 5 || freq == n - 5, 
                "Expected frequency 5 or {}, got {}", n - 5, freq);
    }
    ...
    fn test_harmonic_stability() {
        ...
        assert!(purity_diff < 0.01, 
                "Harmonic purity should be stable: diff={}", purity_diff);
        
        // Should have similar number of dominant frequencies
        let freq_diff = (analysis1.dominant_frequencies.len() as i32 - 
                        analysis2.dominant_frequencies.len() as i32).abs();
        assert!(freq_diff <= 2, 
