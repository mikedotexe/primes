...
fn test_membrane_prime_harmonics() {
    ...
    println!(
        "\nMembrane configuration (3,7) k=(0,0) generated {} primes out of {} valid numbers",
        prime_count, total_count
    );
...
fn test_harmonic_identity_validation() {
    ...
    println!(
        ...
        prime_analysis.dominant_frequencies.len()
    );
    ...
    println!(
        "  Dominant frequencies: {} found",
        random_analysis.dominant_frequencies.len()
    );

    println!(
        "Purity ratio (prime/random): {:.2}x",
