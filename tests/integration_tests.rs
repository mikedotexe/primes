...
use prime_physics_engine::{
    ...
    prime_sieve::{BitSieve, segmented_sieve, warm_slc, sieve_count_and_time},
};
...
fn test_cache_warming() {
    ...
    let monitor = PerfMonitor::new();
    let result3 = monitor.time("cache_warm_test", || {
        warm_slc(100_000, 0.5)
    });
...
fn test_prime_validation_accuracy() {
    ...
    for c in composites {
        ...
        assert!(!prime_physics_engine::is_prime(&n), "{} should be composite", c);
    }
}
