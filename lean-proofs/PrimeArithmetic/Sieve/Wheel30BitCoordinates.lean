...
def wheel30SegmentSlots : ℕ := wheel30SegmentBytes * 8
...
theorem wheel30LinearIndex_lt_segmentSlots {cycle : ℕ} (slot : Fin 8)
  ...
  have h2 : cycle * 8 + 8 ≤ wheel30SegmentBytes * 8 := by
    calc
      cycle * 8 + 8 = (cycle + 1) * 8 := by omega
      _ ≤ wheel30SegmentBytes * 8 := by
...
theorem wheel30ByteIndex_candidate {base cycle : ℕ} (slot : Fin 8) :
  ...
  simp [wheel30ByteIndex, wheel30Index_candidate, wheel30LinearIndex_byte]
...
theorem wheel30BitIndex_candidate {base cycle : ℕ} (slot : Fin 8) :
  ...
  simp [wheel30BitIndex, wheel30Index_candidate, wheel30LinearIndex_bit]
...
theorem wheel30BitCoordinates_candidate {base cycle : ℕ} (slot : Fin 8) :
  ...
  simp [wheel30BitCoordinates, wheel30Index_candidate, wheel30LinearIndex_byte,
    wheel30LinearIndex_bit]
...
theorem wheel30MarkQuery_same_coordinates_candidate {base cycle : ℕ} (slot : Fin 8) :
  ...
  simp [wheel30BitCoordinates, wheel30Index_candidate]
