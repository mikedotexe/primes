...
| maintained matched-control lanes have a generated key lookup surface | the Rust maintained smoke-panel catalog exports exact bounded-`k` lane constants, a duplicate-free `(familyCode, middleWidth)` key list, and generated lookup theorems showing each maintained key resolves to its corresponding lane | `PrimeArithmetic/Generated/MatchedControlFamilyLanes`, `src/bin/export_matched_control_family_lanes.rs` | proved/generated |
| canonical smoke lanes map to exact singleton residue profiles | stable smoke-panel family codes are imported from the Rust-generated bounded-`k` family-lane catalog; reusable profile certificates package each lookup-resolved lane, coprime modulus, and excluded seed class into profile and divisibility-iff facts; every one-modulus coprime lane mask excludes exactly one residue class and therefore has `modulus - 1` survivor classes; base-`6` excludes seed classes `1,0` mod `7` at `M = 1,2`; base-`10` `(3,3)` `M = 2` breathing/symmetric lanes exclude seed classes `3,5` mod `7`, and `(3,7)` `M = 2` classic/exclusive lanes exclude seed classes `5,0` mod `7`; generated base-`10` M2 pair theorems cover the same-boundary pairs and the cross-boundary breathing/classic, symmetric/exclusive, breathing/exclusive, and symmetric/classic rows, with the symmetric/classic distinction certified at mod `3`, and now state both finite forbidden-residue-set separation and seed-mask separation; base-`12` excludes `2,1` mod `5`; base-`14` excludes `4,0` mod `5`; and base-`30` excludes `4,2` mod `7` | `PrimeArithmetic/Generated/MatchedControlFamilyLanes`, `PrimeArithmetic/Structure/BoundedKResidueProfile`, `PrimeArithmetic/Density/CanonicalSmokeLaneProfiles`, `PrimeArithmetic/Generated/MatchedControlSmokeProfileCertificates` | proved |
| base-10 maintained lanes can separate forbidden seed classes | stable canonical-smoke base-`10`, middle width `M = 1` lane pairs are imported from the Rust-generated bounded-`k` family-lane catalog; singleton mod-`11` profiles recover exact excluded seed classes: `(3,3)` with `k = (0,1)` vs `k = (1,1)` gives `0` vs `10`, `(3,7)` with `k = (0,0)` vs `k = (1,1)` gives `8` vs `2`, cross-boundary pairs from `(3,3), k = (1,1)` to `(3,7), k = (1,1)` / `(0,0)` give `10` vs `2` / `8`, and cross-boundary pairs from `(3,3), k = (0,1)` to `(3,7), k = (1,1)` / `(0,0)` give `0` vs `2` / `8`; the corresponding finite forbidden-residue sets and local seed masks are distinct, while same-modulus survivor counts remain equal | `PrimeArithmetic/Generated/MatchedControlFamilyLanes`, `PrimeArithmetic/Structure/AffineSeedClasses`, `PrimeArithmetic/Structure/BoundedKResidueProfile`, `PrimeArithmetic/Density/Base10SeedClassSeparation` | proved |
| proof-carrying witness certificates can expose Lean-checkable construction rows | every tracked witness certificate in the canonical bundle is generated into a concrete base-`10`, `(3,7)`, `k = (2,1)` affine template module; Lean proves exact width, shift, gradient, witness value, small-prime residue rows, rejection examples, bounded search-replay residue facts, and aggregate residue-funnel survival for the generated modules; each generated witness module now contains a `SearchReplayCertificate` proof object showing that pre-witness replay rows are residue-rejected or explicit non-witness survivors, that the accepted witness row survives the finite residue funnel, that the survivor list is exactly the residue-survivor subset of the finite replay window, that residue-rejected rows plus survivor rows form a disjoint replay partition with scanned/rejected/survivor counts matching certificate metadata, and that every pre-witness residue survivor is explicitly non-accepted while the witness seed is the first accepted residue survivor; the promoted policy-matrix tranche adds generated Lean replay modules for all six small matrix rows plus all six 64-digit matrix rows from tracked matrix certificates; the 96-digit matrix rows remain atlas-only next-target candidates, with large replay windows exposing aggregate replay theorem links rather than per-seed convenience wrappers; the theorem-facing teaching wrapper only forwards compact human-facing theorem names; no module claims a primality proof, and the witness Lean catalog manifests map JSON artifacts to generated modules and theorem names | `PrimeArithmetic/Generated/Witness/Seed60`, `PrimeArithmetic/Generated/Witness/Teaching38`, `PrimeArithmetic/Generated/Witness/TimestampPolicy29Trial0`, `PrimeArithmetic/Generated/Witness/MatrixDecimalReadable22`, `PrimeArithmetic/Generated/Witness/MatrixDecimalClassic22`, `PrimeArithmetic/Generated/Witness/MatrixDecimalBreathing22`, `PrimeArithmetic/Generated/Witness/MatrixDecimalReadable64`, `PrimeArithmetic/Generated/Witness/MatrixDecimalClassic64`, `PrimeArithmetic/Generated/Witness/MatrixDecimalBreathing64`, `PrimeArithmetic/Generated/Witness/MatrixBase30Wheel64`, `PrimeArithmetic/Generated/Witness/MatrixBase6Compact18`, `PrimeArithmetic/Generated/Witness/MatrixBase12Compact18`, `PrimeArithmetic/Generated/Witness/MatrixBase6Compact64`, `PrimeArithmetic/Generated/Witness/MatrixBase12Compact64`, `PrimeArithmetic/Generated/Witness/MatrixBase30Wheel18`, `PrimeArithmetic/Witness/SearchReplayCertificate`, `PrimeArithmetic/Witness/TeachingSeedCertificate`, `docs/witness/witness_lean_catalog_manifest.json`, `docs/witness/witness_policy_matrix_lean_catalog_manifest.json` | proved generated construction/replay residue facts |
...
| direct bounded-`k` lane comparisons have an exact local profile object | for fixed base, pair, middle width, and lane, each coprime modulus contributes a precise forbidden seed class, lane agreement can be stated as equality of those local profiles, and a checked `templateValue` zero witness certifies the corresponding `zeroSeedClassAt` equality | `PrimeArithmetic/Structure/BoundedKResidueProfile`, `PrimeArithmetic/Generated/BoundedKResidueProfileShell` | proved |
...
| whole connector families inherit the same residue logic | once the left/right pair has a known residue class modulo `m`, forward and reverse concatenations reduce to a generic connector-shift profile, giving reusable admissibility lemmas for the entire family; for a shared finite connector list this also proves equal forward/reverse residue-survivor counts, so the exact residue layer is a null explanation for directional prime-hit gaps | `PrimeArithmetic/Connector/ConcatenationFamilies` | proved |
| the connector-family API works on maintained non-canonical pairs | the same family theorems instantiate cleanly on the zero-padded membrane pair, a twin-prime pair, and a Sophie Germain pair, producing explicit `mod 3` / `mod 9` connector classes and named equal survivor-count wrappers beyond the canonical asymmetry example; the selected twin-prime residual target also has a combined `mod 3`/`mod 9` survivor-null wrapper; for any twin-prime pair `(p, p + 2)` with `3 < p`, decimal connectors divisible by `3` force both forward and reverse concatenations to be divisible by `3`, explaining why the `(3,5)` / `900000` stress peak is pair-specific under nearby twin controls; every replicated digit-8 stress cell now has a bounded finite classifier for its best separator: leading `800000` uses `p % 17 ∈ {0, 3}`, trailing `00008` uses `p % 19 ∈ {6, 10, 11, 12}`, and trailing `000008` uses `p % 17 ∈ {1, 13, 16}`; report-selected follow-on classifiers now add leading `800000` at `p % 23 ∈ {2, 3, 17}`, `p % 29 ∈ {3, 13, 17}`, and `p % 31 ∈ {3, 9, 17}`, trailing `00008` at `p % 29 ∈ {0, 11, 14, 20}` and `p % 31 ∈ {8, 11, 14, 29}`, and trailing `000008` at `p % 19 ∈ {4, 6, 16}`, `p % 29 ∈ {4, 14, 21}`, and `p % 31 ∈ {8, 13, 25}`; each of the three digit-8 edge cells also has a compact multi-modulus classifier theorem bundling its row facts; none of these statements claims a density mechanism | `PrimeArithmetic/Connector/ConcatenationProfileExamples` | proved |
...
| the analytic layer can be stated conservatively in classical notation | pair-count conventions, odd-prime Goldbach local factors, radical invariance of the local-factor support, and the standard logarithmic and Poisson-style transforms can be fixed in Lean; the Poisson coverage transform is monotone in supplied expected-hit `λ`, and ordered/unordered coverage comparisons are formally convention-driven when supplied unordered `λ` is nonnegative or positive, but this is not a density-mechanism theorem | `PrimeArithmetic/Analysis/HardyLittlewoodShell` | proved |
...
- a conservative Hardy-Littlewood notation shell with exact local-factor support
  bookkeeping and monotone expected-hit-to-coverage transforms
...
### Current Matched-Control Queue Anchor

The tracked theorem queue at
[`../docs/atlas/matched_control_theorem_queue.md`](../docs/atlas/matched_control_theorem_queue.md)
currently selects the canonical smoke base-`10`, `(3,3)`, `M = 1` pair
`k = (0,1)` versus `k = (1,1)`. Its existing Lean proof links are:
...
The generic finite-product residue-mask overlap helper now lives in
`PrimeArithmetic.Structure.BoundedKResidueProfile`: given a finite list of
coprime nonzero moduli, equal individual survivor products follow, and the
shared-survivor product is determined locally by whether the two singleton
forbidden classes agree or differ at each modulus. The next non-duplicative
theorem shape is a concrete instantiation for the queue-selected pair, lifting
the scanner's exact fingerprint geometry into Lean while still making no
prime-density claim.
